...
export function getResults(formValues) {
  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      resolve(allBestPractices)
    }, DELAY)
  })
}

export function getBestPractices(number) {
  const practices = []
  const practicesLen = Math.min(number, allBestPractices.length)

  for (let i = 0; i < practicesLen; i++) {
    practices.push(allBestPractices[i])
  }
  return practices
}

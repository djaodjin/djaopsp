...
#                    " from '%s'" % (method, path, docstring))
        ...
        if examples:
            for example in examples:
                ...
                if not example['path']:
                    ...
                    # XXX ending with {path} will always fail here.
    ...
    def get_request_body(self, path, method):
        ...
        if not hasattr(view, 'request'):
            view.request = HttpRequest(DangoHttpRequest())
...
class APIDocView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        schema = generator.get_schema(request=HttpRequest(DangoHttpRequest()),
            public=True)
        tags = set([])
